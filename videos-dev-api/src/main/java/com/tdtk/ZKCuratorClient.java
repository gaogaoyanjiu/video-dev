...
//	public static final String ZOOKEEPER_SERVER = "192.168.200.201:2181";
	...
	public void init() {
		...
		try {
//			String testNodeData = new String(client.getData().forPath("/bgm/181019BNTFA826NC"));
...
//					String bgmId = arr[arr.length - 1];//得到最后一级的子节点路径
...
//					String songPath = bgm.getPath();
					...
					// 2. 定义保存到本地的bgm路径
//					String filePath = "E:\\videos_dev" + songPath;
					String filePath = resourceConfig.getFileSpace() + songPath;
					
					// 3. 定义下载的路径（播放url）
					String arrPath[] = songPath.split("\\\\");//对数据库的路径的反斜杠转义后为四个反斜杠
					String finalPath = "";
					// 3.1 处理url的斜杠以及编码
...
//					String bgmUrl = "http://localhost:8080/mvc" + finalPath;
					...
					if (operatorType.equals(BGMOperatorTypeEnum.ADD.type)) {
						...
						try{
							FileUtils.copyURLToFile(url, file);//下载操作，必须启动admin管理的项目，admin服务器需要配置（非maven启动）：URIEncoding="UTF-8"
